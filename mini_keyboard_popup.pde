...
void setup()
{
  ...
  textFont(createFont("Arial", 14)); //set the font to arial 24. Creating fonts is expensive, so make difference sizes once in setup, not draw
...
void draw()
{
  ...
  if (startTime!=0)
  {
    ...
    // draw keyboard
    float letWidth = DPIofYourDeviceScreen / 5;
    float letHeight = DPIofYourDeviceScreen / 6;
    float letHalfWidth = letWidth / 2;
    float letHalfHeight = letHeight / 2;
    float offsetW = width/2 - DPIofYourDeviceScreen/2;
    float offsetH = height/2 - DPIofYourDeviceScreen/2;
    ...
    for (int i = 0; i < 6; i++) {
      for (int j = 0; j < 5; j++) {
        if (i == 5 && j > 0) { // special keys
          if (j == 1) { // space
            fill(255, 255, 255);
            rect(offsetW + j * letWidth, offsetH + i * letHeight, letWidth * 3, letHeight);
            fill(0);
            text("space", offsetW + (j + 1.5) * letWidth, offsetH + i * letHeight + letHalfHeight);
          } else if (j == 4) { // backspace
            fill(255, 0, 0);
            rect(offsetW + j * letWidth, offsetH + i * letHeight, letWidth, letHeight);
          }
        }
        //pressed key
        else if (coords[0]==i && coords[1]==j){
          if (i==0 && j==0){
            fill(0,100,0);
            rect(offsetW + j * letWidth, offsetH + i * letHeight, letWidth, letHeight);
            fill(255);
            text("" + currentLetter , offsetW + j * letWidth + letHalfWidth, offsetH + i * letHeight + letHalfHeight);
            currentLetter++;}
          
          else if (i==0 && j>0){
            fill(0,100,0);
            rect(offsetW + j * letWidth-letWidth, offsetH + i * letHeight, letWidth*2, letHeight);
            fill(255);
            text("" + currentLetter , offsetW + j * letWidth + letHalfWidth-letWidth, offsetH + i * letHeight + letHalfHeight);
            currentLetter++;}
          else {
            fill(0,100,0);
            rect(offsetW + j * letWidth, offsetH + i * letHeight - letHeight, letWidth, letHeight*2);
            fill(255);
            text("" + currentLetter, offsetW + j * letWidth + letHalfWidth, offsetH + i * letHeight + letHalfHeight - letHeight);
            currentLetter++;
          }
        }
        else { // other letters
    ...
    if (coords[0]==i && coords[1]==j){
      if (i==0 && j==0){
        fill(0,100,0);
        rect(offsetW + j * letWidth, offsetH + i * letHeight, letWidth*2, letHeight);
        fill(255);
        text("" + currentLetter , offsetW + j * letWidth + letHalfWidth+letWidth, offsetH + i * letHeight + letHalfHeight);
        currentLetter++;}
    }
  ...
  for (int i = 0; i < 6; i++) {
    for (int j = 0; j < 5; j++) {
      if (i == 5 && j > 0) { // special keys
        ...
        if (didMouseClick(offsetW + j * letWidth, offsetH + i * letHeight, letWidth, letHeight)) {
          coords[0]=i;
          coords[1]=j;
        }
      }
...
void mouseReleased()
{
  ...
  for (int i = 0; i < 6; i++) {
    for (int j = 0; j < 5; j++) {
      if (i == 5 && j > 0) { // special keys
        if (j == 1) { // space
          if (didMouseClick(offsetW + j * letWidth, offsetH + i * letHeight, letWidth * 3, letHeight)) {
            currentTyped += " ";
          }
        } else if (j == 4) { // backspace
          if (didMouseClick(offsetW + j * letWidth, offsetH + i * letHeight, letWidth, letHeight)) {
